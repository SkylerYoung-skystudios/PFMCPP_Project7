...
void useDefensiveItem(Character* character, Item& item)
{
    ...
    else if( auto* chPal = dynamic_cast<Paladin*>(character) )
    {
        //same with paladins
        chPal->boostArmor( item.getBoost() * 1.3 );
    }
    else if( auto* chSlay = dynamic_cast<DragonSlayer*>(character))
    {
        chSlay->boostArmor( item.getBoost() * 1.5 );
    }
    else if( auto* chDrag = dynamic_cast<Dragon*>(character) )
    {
...
void useHelpfulItem(Character* character, Item* item)
{
    ...
    {
        chPal->boostHitPoints(item->getBoost() * 1.5);
    }
    else if( auto* chSlay = dynamic_cast<DragonSlayer*>(character))
    {
        chSlay->boostHitPoints(item->getBoost() * 1.25);
    }
    else if( auto* chDrag = dynamic_cast<Dragon*>(character) )
    {
...
void useAttackItem(Character* character, Item* item)
{
    ...
    {
        chPal->boostAttackDamage(item->getBoost() * 1.33);
    }
    else if( auto* chSlay = dynamic_cast<DragonSlayer*>(character))
    {
        //assert(false);
        ...
        chSlay->boostAttackDamage(item->getBoost() * chSlay->getAttackDamage());
    }
    else if( auto* chDrag = dynamic_cast<Dragon*>(character) )
    {
